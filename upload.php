...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  if (isset($_FILES['pdf']) && $_FILES['pdf']['error'] == 0) {
    ...
    $fileType = $_POST['file_type']; // Get the file type (notes or previous papers)

    // Insert file into database
    $stmt = $conn->prepare("INSERT INTO pdf_files (user_id, subject, file_name, file_data, file_type) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("issss", $userId, $subject, $fileName, $fileData, $fileType);

    if ($stmt->execute()) {
      $successMessage = "File uploaded successfully!";
    } else {
      $errorMessage = "Error uploading file: " . $stmt->error;
    }

    $stmt->close();
  } else {
    $errorMessage = "No file selected or file upload error.";
  }
}
...
<body>
  <div class="dashboard-container">
    <h2>Upload PDF for <?php echo strtoupper($subject); ?></h2>
    <form method="POST" enctype="multipart/form-data" class="upload-form">
      ...
      <input type="file" name="pdf" id="pdf-upload" accept="application/pdf" required>
